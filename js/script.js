window.onload = function() {
	...
	$('.intro-img').on('click', function() {
		...
		$('#common-humanity').trigger('play').animate({
			volume: 0.5
		}, 1000);
		$('.intro-container').fadeOut(3000, 'swing');
		//"deblur" e animação da opacidade de cenas
		$('.blur').css({
			"filter": "blur(0px)"
		});
	...
	//navegacao pela navegacao que ja nao é lateral (podia ter dado um nome melhor, confuso)
	...
	//Montes de blocos a correr no scroll
	$(window).on('scroll', function() {
		...
		//Teoria: A percentagem do scroll é = ao scroll do topo da janela a dividir pela diferença entre a altura do documento
		...
		//Parallax eixo y super martelado
	...
	//solução pouco convencional para as empresas que usam AI 
	//!!! Note to self: desativei pointer events no primeiro seletor !!!
